    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 text-gray-100 p-6 border border-emerald-500 rounded-lg">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-4">
                <select class="bg-neutral-900 border border-neutral-700 rounded-lg px-4 py-2 text-sm text-white" onchange="window.location.href='{{ route('forum.category', $category->slug) }}?sort=' + this.value">
                    <option value="hot" {{ $sort === 'hot' ? 'selected' : '' }}>Sort by: Hot</option>
                    <option value="latest" {{ $sort === 'latest' ? 'selected' : '' }}>Sort by: New</option>
                    <option value="top" {{ $sort === 'top' ? 'selected' : '' }}>Sort by: Top</option>
                </select>
                <a href="{{ route('forum.category.create-topic', $category->slug) }}" class="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm transition-all duration-300 ease-in-out hover:scale-[1.02] flex items-center gap-2">
        ...
        <div class="mt-6 space-y-4">
            @if($topics->isEmpty())
                <div class="bg-neutral-800 border border-neutral-700 rounded-lg p-8 text-center">
                    <p class="text-neutral-400">No topics found in this category.</p>
                    <a href="{{ route('forum.category.create-topic', $category->slug) }}" class="mt-4 inline-block bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm transition-all duration-300 ease-in-out hover:scale-[1.02]">
                ...
                @foreach($topics as $topic)
                    <div class="bg-linear-to-br from-neutral-800 to-neutral-900 border border-neutral-700 overflow-hidden shadow-xs rounded-xl transition-all duration-300 ease-in-out hover:border-neutral-600">
                        <div class="p-6">
                            ...
                            <!-- Topic Content -->
                            <a href="{{ route('forum.topic', [$category->slug, $topic->slug]) }}" class="block">
