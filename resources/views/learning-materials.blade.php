        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            @if($materials->count() > 0)
                ...
                <div class="bg-neutral-800 rounded-lg shadow-lg p-6 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] border border-neutral-700">
                    <div class="mb-4 flex items-center justify-between">
                        <span class="rounded-full bg-emerald-900 px-3 py-1 text-xs font-medium text-emerald-200">{{ strtoupper(pathinfo($material->file_name, PATHINFO_EXTENSION)) }}</span>
                    ...
                    <div class="mb-4 flex items-center space-x-2 text-sm text-neutral-400">
                        ...
                        <span>•</span>
                        <span>Last updated: {{ $material->updated_at->format('M d, Y') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            <a href="{{ asset('storage/' . $material->file_path) }}" target="_blank" class="inline-flex items-center space-x-1 text-sm text-blue-400 hover:text-blue-300 transition-colors duration-300">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                <span>Preview</span>
                            </a>
                        </div>
                        <a href="{{ asset('storage/' . $material->file_path) }}" download class="inline-flex items-center space-x-1 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-neutral-800 transition-colors duration-300">
            ...
            @else
                <div class="col-span-full text-center py-12">
                    <p class="text-lg text-neutral-400">No Learning Materials Available</p>
                </div>
            @endif
        </div>
        <br>
