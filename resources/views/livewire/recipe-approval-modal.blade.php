<div
    wire:poll.1s="checkForApprovedRecipes"
    x-data="{
        showApprovalModal: @entangle('showApprovalModal')
    }"
    x-init="
        // Check for approved recipes on page load and set up interval
    ...
    <div x-show="showApprovalModal"
        ...
        <div class="relative bg-neutral-800 rounded-xl shadow-lg p-8 max-w-md w-full mx-4 border border-emerald-500/30 transform transition-all max-h-[90vh] overflow-y-auto"
            ...
            <div class="text-center">
                ...
                <h3 class="text-xl font-bold text-white mb-2">Recipe Approved!</h3>
                <p class="text-neutral-300 mb-4">Your recipe "{{ $recipeName }}" has been reviewed and approved by your teacher.</p>

                @if($description)
                <div class="bg-neutral-700/50 rounded-lg p-4 mb-4 text-left border border-neutral-600">
                    <h4 class="text-sm font-medium text-emerald-400 mb-2">Your Recipe Description:</h4>
                    <div class="bg-neutral-800/70 p-3 rounded border border-neutral-700">
                        <p class="text-sm text-neutral-300 whitespace-pre-line">{{ $description }}</p>
                    </div>
                </div>
                @endif
                ...
                <div class="mt-4 text-center">
                    <button wire:click="checkForApprovedRecipes" class="text-xs text-neutral-400 hover:text-neutral-300">
                        Check for approved recipes
                    </button>
                </div>
                @endif
            </div>
