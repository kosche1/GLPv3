    ...
    public function handle()
    {
        ...
        try {
            ...
            $insertResult = DB::table('student_attendance')->insert([
                ...
                'date' => $today,
                'status' => 'present',
                'notes' => 'Test attendance record at ' . $currentTime,
            ...
            if ($finalRecord) {
                ...
                $this->table(['ID', 'User ID', 'Date', 'Status', 'First Login', 'Last Login', 'Count', 'Notes'], [
                    [
                        $finalRecord->id,
                        $finalRecord->user_id,
                        $finalRecord->date,
                        $finalRecord->status,
                        $finalRecord->first_login_time ?? 'N/A',
                        $finalRecord->last_login_time ?? 'N/A',
                        $finalRecord->login_count ?? '1',
                        $finalRecord->notes
                    ]
                ]);
            } else {
